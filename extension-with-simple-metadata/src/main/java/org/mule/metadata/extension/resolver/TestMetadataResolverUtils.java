...
import org.mule.metadata.extension.MetadataConnection;
...
public class TestMetadataResolverUtils
{
    ...
    public static Set<MetadataKey> getOtherKeys(MetadataContext context) throws ConnectionException
    {
        return Arrays.asList("Key1", "Key2", "Key3").stream().map(e -> MetadataKeyBuilder.newKey(e).build()).collect(toSet());
    }
